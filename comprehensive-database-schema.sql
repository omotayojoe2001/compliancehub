-- Comprehensive ComplianceHub Database Schema
-- This replaces all mock data with real database functionality
...
CREATE TABLE IF NOT EXISTS reminder_logs (
  ...
  reminder_type TEXT CHECK (reminder_type IN ('email', 'whatsapp', 'sms')),
...
CREATE INDEX IF NOT EXISTS idx_tax_obligations_status ON tax_obligations(payment_status);
...
CREATE POLICY "Users can view own reminder logs" ON reminder_logs FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own tax calculations" ON tax_calculations FOR ALL USING (auth.uid() = user_id);
